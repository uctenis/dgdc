...
export const ProjectManager: React.FC<ProjectManagerProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            ...
            <span>Gesti├│n de Proyectos & Licitaciones de Obra</span>
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            Administre los procesos de compra y licitaci├│n identificados por sus c├│digos CP, OP y OT.
          </p>
        </div>
        ...
        >
          ...
          <span>Crear Nueva Licitaci├│n</span>
        </button>
      </div>
            ...
            >
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                    ...
                    <span className="font-bold text-sky-700 bg-sky-100 px-2 py-0.5 rounded font-mono">
                      C├│d: {lic.codigoProyecto}
                    </span>

                    {/* OT y OP solo si ya se adjudic├│ el proyecto */}
                    {lic.codigoOT ? (
                    ...
                    {!lic.codigoOT && !lic.codigoOP && (
                      <span className="text-[10px] text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200 font-medium">
                        ÔÅ│ OT / OP: Asignaci├│n al Adjudicar
                      </span>
                    )}
                    ...
                    {lic.campusSigla && (
                      <span className="font-semibold text-slate-700 bg-slate-100 px-2 py-0.5 rounded flex items-center gap-1 border border-slate-200">
                        ...
                        {lic.campusSigla} {lic.edificioSigla ? `ÔÇó ${lic.edificioSigla}` : ''}
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="bg-slate-900 text-white p-3 rounded-xl space-y-2 border border-slate-800">
                  ...
                  {cotsLic.length === 0 ? (
                    <p className="text-[10px] text-slate-400 italic">No hay ofertas ingresadas a├║n. Presione "+ Ingresar Oferta" para agregar.</p>
                  ) : (
                ...
                {/* Secci├│n Orden de Compra (OC) Post-Adjudicaci├│n */}
                {lic.proveedorAdjudicadoId && (
                  <div className="p-3 bg-purple-50 rounded-xl border border-purple-200 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      <span className="p-1.5 bg-purple-200 text-purple-900 rounded-lg font-bold text-[10px]">­ƒÆ│ OC</span>
                      <div>
                        {lic.ordenCompraNumero ? (
                          ...
                          <>
                            ...
                            <span className="text-[10px] text-purple-600">Suba el PDF/Excel de la OC para leerla autom├íticamente</span>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    >
                      {lic.ordenCompraNumero ? 'Reemplazar OC' : 'Subir OC ÔåÆ'}
                    </button>
                  </div>
                )}
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title="Ver y editar Acta de Evaluaci├│n Propuesta y Adjudicaci├│n SGC"
                    >
      ...
      {/* Acta de Evaluaci├│n Propuesta Modal */}
      {actaLicitacion && (
        <ActaEvaluacionModal
      ...
      {/* Antecedentes T├®cnicos & Checklist Modal */}
            ...
            <div className="flex items-center justify-between border-b pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Seleccione el proyecto presupuestado del a├▒o 2026 para autocompletar su informaci├│n.
                </p>
              </div>
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="text-lg font-bold text-slate-800">
                {editingId ? 'Editar Licitaci├│n' : 'Crear Nueva Licitaci├│n'}
              </h3>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200">
                <div>
                  <label className="block font-semibold text-slate-700 mb-1">C├│d. Proyecto *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block font-semibold text-slate-500 mb-1">C├│digo OT (Orden Trabajo)</label>
                  <input
                    type="text"
                    disabled
                    value={codigoOT ? codigoOT : 'Pendiente (Post-Adjudicaci├│n)'}
                ...
                <div>
                  <label className="block font-semibold text-slate-500 mb-1">C├│digo OP (Orden Pedido)</label>
                  <input
                    type="text"
                    disabled
                    value={codigoOP ? codigoOP : 'Pendiente (Post-Adjudicaci├│n)'}
                    className="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-slate-500 outline-none text-[11px] font-semibold"
                  />
                  <span className="text-[10px] text-amber-700 font-semibold mt-0.5 block">Se asigna en Administraci├│n</span>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Remodelaci├│n Laboratorio de Redes Campus San Juan Pablo II"
              ...
              <div>
                <label className="block font-semibold text-slate-700 mb-1">Descripci├│n del Requerimiento</label>
                <textarea
                  rows={3}
                  {...ATRIBUTOS_ORTOGRAFIA_ES}
                  placeholder="Detalles de la obra, ubicaci├│n y alcances..."
                  value={descripcion}
              ...
              {/* Calendario de la Licitaci├│n (SGC) */}
              <div className="bg-sky-50/60 p-3.5 rounded-xl border border-sky-100 space-y-2">
                <span className="text-[11px] font-bold text-sky-900 block flex items-center gap-1">
                  ...
                  Calendario SGC de la Licitaci├│n (Hitos Obligatorios)
                </span>
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="block font-semibold text-slate-700 mb-1 text-[11px]">2. Recepci├│n Consultas</label>
                    <input
                      type="date"
                      required
              ...
              <div className="flex items-center justify-between pt-3 border-t">
                {editingId ? (
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm('┬┐Confirma que desea eliminar definitivamente esta licitaci├│n y todos sus datos cargados?')) {
                ...
                <div className="flex items-center space-x-3">
                  ...
                  >
                    {editingId ? 'Guardar Cambios' : 'Crear Licitaci├│n'}
                  </button>
                </div>
              </div>
            </form>
